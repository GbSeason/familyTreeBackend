...
def generate_data(file_dir):
    ...
    if os.path.exists(file_dir):
        ...
        for f in file_list:
            ...
            image_g = image_preprocessing(cv2.imread(img_name))
            img = image_g.astype(np.float32)
            img = cv2.resize(img, dsize=(50, 50), interpolation=cv2.INTER_CUBIC)
            new_img = img.reshape(1, 50 * 50)
...
def image_preprocessing(image):
    ...
    image_preprocess = cv2.GaussianBlur(imageGray, (3, 3), sigmaX=1, sigmaY=1)
...
def SVM_train(model, samples, responses):
    ...
    model.save("C:/familyTree/backend/util/SVM_MLPTrainCase.xml")
    return model
...
if __name__ == "__main__":
    print('加载图片...')
    # load_image('C:/familyTree/backend/util/img/*.png')
    datas, labels = generate_data("C:/familyTree/backend/util/img/")

    temp, data_test = np.split(datas, [9])
    ...
    data_train, temp = np.split(datas, [9])
    labels_train, labels_test = np.split(np.array(labels), [9])
    hog_vector_train, hog_vector_test = np.split(hog_vector, [9])
